...
def _build_skill_prompt(
    ...
    return (
        ...
        '\n'
        f'{_format_prompt_tail(content, suggestions, user_instruct, _SKILL_EDIT_OUTPUT_SPEC, previous_error)}'
    )
...
def _parse_skill_operations(payload: Dict[str, Any]) -> List[Dict[str, Any]]:
...
def _split_skill_frontmatter(content: str) -> tuple[str, str, str]:
...
def _apply_frontmatter_update(current: str, fields: Dict[str, Any]) -> str:
    opening, yaml_text, closing, body = _split_skill_frontmatter(current)
    ...
    for idx, line in enumerate(lines):
        ...
        if key not in fields:
            continue
        lines[idx] = f'{match.group(1)}{key}{match.group(3)}{_format_yaml_scalar(fields[key])}'
        consumed.add(key)
...
def _apply_section_edit(current: str, heading: str, content: str, mode: str) -> str:
...
def _apply_skill_edit_operations(current_content: str, payload: Dict[str, Any]) -> str:
    operations = _parse_skill_operations(payload)
    ...
    for op in operations:
        if op['op'] == 'replace_text':
            current = _apply_replace_text(current, op['old'], op['new'], entity_name='skill')
            continue
        if op['op'] == 'update_frontmatter':
            current = _apply_frontmatter_update(current, op['fields'])
            continue
        if op['op'] == 'replace_section':
            current = _apply_section_edit(current, op['heading'], op['content'], op['mode'])
            continue
