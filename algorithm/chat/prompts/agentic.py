...
MEMORY_GUIDANCE = (
    ...
    "When using the memory tool, pass JSON edit operations against the current full target text. "
    "Use replace_text only when the old text is an exact substring; use replace_all when adding entries, deduplicating, resolving conflicts, or reorganizing the full text. "
...
SEARCH_GUIDANCE = (
    ...
    "may you fall back to `web_search`, `url_fetch`, or `sciverse_search`. "
    ...
    "insufficient, prefer `sciverse_search` over `web_search`.\n\n"
    "When answering with knowledge-base evidence, cite with the original `[[document.chunk]]` "
    "markers. When answering with `web_search`, `url_fetch`, or `sciverse_search`, do not "
...
_MEMORY_REVIEW_PROMPT = (
    ...
    "For user_preference changes (target='user_preference'): save user-stated identity, role, preferred "
    ...
    "Call memory(target='memory', operations=[...]) only for: concise agent working-memory notes about the user's ongoing work, "
    "what was discussed, and other future-relevant session context. The operations must edit the current full memory text into the desired reviewed full text. "
    "Prefer timestamped phrasing when known, ideally in `YYYY-MM-DD: ...` form.\n"
    "Call memory(target='user_preference', operations=[...]) for: user identity/role, "
    ...
    "- Base your edit operations on the existing content; do NOT propose a blind rewrite from scratch.\n"
    ...
    "- Remove or update only what is proven outdated or wrong; express the final intended text through edit operations.\n"
    ...
    "- The memory tool applies operations and writes one pending memory_review row; it does NOT directly overwrite the final stored memory or user_preference.\n"
    ...
    "If there is a worthwhile memory or user_preference change, directly call the memory tool to submit the pending review row. "
...
_COMBINED_REVIEW_PROMPT = (
    ...
    "For user_preference updates (target='user_preference'): save user-stated identity, role, "
